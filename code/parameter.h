...
#define MORE_ANEOS_OUTPUT 0 // produce additional output to HDF5 files (T, cs, entropy); only useful when HDF5IO is set; set only if you use the ANEOS eos, but currently not supported for porosity+ANEOS
#define OUTPUT_GRAV_ENERGY 0    // compute and output gravitational energy (at times when output files are written); of all SPH particles (also w.r.t. gravitating point masses, but not between gravitating point masses); direct particle-particle summation, not tree; option exists to control costly computation for high particle numbers
